...
import { addBlockToParent, deleteBlockById, duplicateBlock, moveBlock, updateBlockById, validateBlockStructure } from '../utils/ContentUtils';
...
export function useNestedContent(initialBlocks: NestedContentBlock[] = []): UseNestedContentReturn {
        ...
        setBlocks((prevBlocks) => {
            const updatedBlocks = prevBlocks.map((block) => {
                if (block.id === parentId && block.children) {
                    ...
                    const updatedChildren = block.children.map((child) => (child.id === childId ? { ...child, ...updates } : child));
    ...
    const deleteBlockChild = useCallback(
    ...
    const getBlockTypeCount = useCallback(
    ...
    const hasBlocksOfType = useCallback(
        (type: ContentBlockType) => {
